...
import { AppData, StockMasuk, JasaKupas, Penjualan, RingkasanKeuangan } from './types'
...
export function hitungRingkasan(): RingkasanKeuangan {
  ...
  const totalPengeluaran = totalPembelian + totalJasaKupas
  ...
  const totalJualTidakKupasKg = data.penjualan
    .filter(i => i.jenis === 'tidak_kupas')
    .reduce((s, i) => s + i.berat_kg, 0)
  const totalJualKupasKg = data.penjualan
    .filter(i => i.jenis === 'kupas')
    .reduce((s, i) => s + i.berat_kg, 0)
